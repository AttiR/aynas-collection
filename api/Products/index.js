...
module.exports = async function (context, req) {
    ...
    // Check if this is a request for a specific product (has ID in URL path)
    const urlPath = req.url || '';
    const pathSegments = urlPath.split('/').filter(segment => segment);
    const productId = pathSegments[pathSegments.length - 1];

    // Check if the last segment is a number (product ID)
    const isProductByIdRequest = !isNaN(productId) && productId !== '';

    context.log('URL path:', urlPath);
    context.log('Path segments:', pathSegments);
    context.log('Product ID:', productId);
    context.log('Is product by ID request:', isProductByIdRequest);

    if (isProductByIdRequest) {
        // Handle individual product request
        context.log('Handling individual product request for ID:', productId);
        ...
        try {
            ...
            context.log('Error fetching from .NET API, using hardcoded data:', error.message);
        }

        // Use hardcoded data as fallback for individual product
        const allProducts = [
            {
                id: 1,
    ...
    // Handle products list request (original functionality)
